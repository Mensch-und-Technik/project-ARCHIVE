...
$con = mysqli_connect('mysql.hrz.tu-chemnitz.de', 'Project_ARCHIVE_DB_rw', 'thiwa29e', 'Project_ARCHIVE_DB');
	...
	if(!empty($_POST)){
		...
		//TO-DO: Move delete to task: ReadRecords
		if($_POST["task"] == "ReadRecords"){
			...
			if($numOfRecords > 0){
				...
				//$existingRecords = mysqli_fetch_assoc($checkExists);
				//echo $existingRecords["datapath"];
				//echo mysqli_num_rows($checkExists)  . " object(s) already exist(s); ";	//error code #3 = name exsists
			}
		...
		if($_POST["task"] == "UpdateRecords"){
			...
			//STOP: DO NOT UPDATE but echo saved position and rotation values!
			$insertMultiQuery = "";
